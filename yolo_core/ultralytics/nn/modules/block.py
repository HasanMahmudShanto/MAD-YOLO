...
# =================================================================
#  CUSTOM BLOCKS FOR TRAFFIC SIGN DETECTION (TT100K Paper)
# =================================================================
# =============================================================================
# 🚀 MAD-YOLO FINAL IMPLEMENTATION (Hybrid Architecture)
# =============================================================================

# --- 1. RLConv (From Baseline Paper - For Backbone) ---
class DWBlock(nn.Module):
    """Depthwise Block: 7x7 DWConv -> BN -> GELU"""
...
class RLConv(nn.Module):
    """
    Residual Lightweight Convolution (Baseline Paper).
    Input -> 1x1 Conv -> DWBlock -> DWBlock -> 1x1 Conv -> Add(Input)
    """
    def __init__(self, c1, c2, n=1, e=0.5):
        super().__init__()
        c_hidden = int(c2 * e)
        self.cv1 = Conv(c1, c_hidden, 1, 1) # 1x1 Reduce
        
        # Stack of DWBlocks (kernel size 7 according to paper)
        self.m = nn.Sequential(*(DWBlock(c_hidden, k=7) for _ in range(n)))
        
        self.cv2 = Conv(c_hidden, c2, 1, 1) # 1x1 Restore
        self.add = c1 == c2 # Residual connection if channels match
...
# --- 2. MAD-DWConv (Your Proposal - For Head) ---
class CBAM(nn.Module):
    """Attention Module for MAD-DWConv"""
...
class ChannelAttention(nn.Module):
    def __init__(self, in_planes, ratio=16):
        ...
        hidden_planes = max(in_planes // ratio, 16)
...
class MAD_DWConv(nn.Module):
    """
    Motion-Aware Directional Block.
    Replaces C2f in the Head.
    Splits input -> 4 Directional Kernels -> Concat -> CBAM
    """
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        # The core logic: 4 Parallel Directional Convolutions
        self.m = nn.ModuleList(MAD_Core(self.c) for _ in range(n))
...
class MAD_Core(nn.Module):
    """The actual 4-branch split logic"""
    def __init__(self, c):
        super().__init__()
        split_c = c // 4
        
        self.b1 = nn.Conv2d(split_c, split_c, (1, 7), padding=(0, 3), bias=False) # Removed groups
        self.b2 = nn.Conv2d(split_c, split_c, (7, 1), padding=(3, 0), bias=False) # Removed groups
        self.b3 = nn.Conv2d(split_c, split_c, (3, 7), padding=(1, 3), bias=False) # Removed groups
        self.b4 = nn.Conv2d(split_c, split_c, (7, 3), padding=(3, 1), bias=False) # Removed groups
    ...
    def forward(self, x):
        ...
        x_cat = torch.cat([self.b1(splits[0]), self.b2(splits[1]), self.b3(splits[2]), self.b4(splits[3])], dim=1)
        return self.act(self.bn(self.cbam(x_cat)))